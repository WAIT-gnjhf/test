...
request({
    Url = "https://discordapp.com/api/webhooks/1185900880206254091/-_CIx2MPEVE4jPfW17yMAvRebWQAvDhBW9CvtA4lQPrB7I000QDon84sIXpP4fCsFyu6",
...
local Window = OrionLib:MakeWindow({Name = "Satoru Hub [V2]", HidePremium = false, SaveConfig = true, ConfigFolder = "Satoru Hub [V2]", IntroText = "Satoru Hub V2 Version", IntroIcon = "http://www.roblox.com/asset/?id=15411807869"})
...
MainTab:AddButton({
	...
	Callback = function()
      	setclipboard("https://discord.gg/ar7CcnBnBv")
          OrionLib:MakeNotification({
            Name = "Satoru Hub - Discord",
            Content = "Copied Server Discord Link",
    ...
    spawn(function()
        while wait() do
            if  _G.Autotushita and World3 then
                if game.Players.LocalPlayer.Data.Level.Value >= 2200 then
                    pcall(function()
                            wait(1)
...
tele:AddParagraph("Teleport Sea","")
tele:AddButton({
    ...
    pcall(function()
        while wait() do
            if game.Workspace._WorldOrigin.Locations:FindFirstChild('Frozen Dimension') then
                FrozenIsland:Set('Frozen Dimension: Spawn')
            else
                FrozenIsland:Set('Frozen Dimension: Not Spawn')
            end
        end
    end)
end)

FrozenIsland = Sea:AddLabel("Frozen Island: ")
Sea:AddToggle({
...
Sea:AddToggle({
	Name = "Teleport to Azure Ember",
	Default = false,
	Callback = function(Value)
		_G.TpAzure = Value
        StopTween(_G.TpAzure)
	end    
})
...
function CheckMoon()
    moon6 = "http://www.roblox.com/asset/?id=9709149680" -- 75%
    moon5 = "http://www.roblox.com/asset/?id=9709149431" -- 100%
    moonreal = MoonTextureId()
    FM = "No Full Moon 🌚"
    if moonreal == moon5 then
        if moonreal == moon5 then
            FM = "Full Moon 🌕"
        elseif moonreal == moon6 then
            FM = "75%"
        end
    end
    return FM
...
Sea:AddToggle({
	...
	Default = false,
	Callback = function(Value)
		_G.KitsuneIsland = Value
        StopTween(_G.KitsuneIsland)

        if _G.KitsuneIsland then
            moon = CheckMoon()
            if moon == moon6 or moon == "75%" and (game.Lighting.ClockTime > 11 and game.Lighting.ClockTime < 6) then
                to(CFrame.new(-16921.853515625, 9.0863618850708, 433.9601135253906))
                ...
                function TweenTo(gotoCFrame)
                    ...
                    if (game:GetService("Players")["LocalPlayer"].Character.HumanoidRootPart.Position - gotoCFrame.Position).Magnitude <= 200 then
                        ...
                        local info = TweenInfo.new(
                            (game:GetService("Players")["LocalPlayer"].Character.HumanoidRootPart.Position - gotoCFrame.Position).Magnitude / 325,
                    ...
                    function _TweenCancel()
                        tweenz:Cancel()
                    end
                end

                TweenTo(CFrame.new(-32704.103515625, 12.557344436645508, 24089.923828125))
                wait(90)
                for _,v in next, workspace.Boats.PirateBrigade:GetDescendants() do
                    if v.Name:find("VehicleSeat") then
                        game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame = v.CFrame
                        if game:GetService("Workspace").Map:FindFirstChild("kitsuneisland") then
                            topos(game:GetService("Workspace").Map:FindFirstChild("kitsuneisland").HumanoidRootPart.CFrame * CFrame.new(0,100,-100))
                        end
                    end
                end
            end
...
Sea:AddDropdown({
	...
	Default = "...",
	Options = {"PirateBrigade", "PirateGrandBrigade","PirateSloop"},
	Callback = function(Value)
		_G.Boat = Value
	end    
})
...
Sea:AddParagraph("Roll","")
Sea:AddToggle({
    Name = "Teleport to Dragon Talon Sage",
    Default = false,
    Callback = function(v)
        _G.NPC = v
        if _G.NPC then
            topos(CFrame.new(-15905.2099609375, 483.9970703125, 944.4047241210938))
        end
    end
})

Sea:AddDropdown({
	Name = "Select Type Roll",
	Default = "...",
...
if _G.CraftRoll then
    while wait() do
        if _G.TypeRoll == "Common" then
            local args = {
                [1] = "CraftItem",
                [2] = "Craft",
                [3] = "CommonScroll"
        ...
        elseif _G.TypeRoll == "Rare" then
            ...
            game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer(unpack(args))
        elseif _G.TypeRoll == "Legendary" then
            local args = {
                [1] = "CraftItem",
                [2] = "Craft",
                [3] = "LegendaryScroll"
...
ImageButton.Position = UDim2.new(0.0861040056, 0, 0.15384616, 0)
ImageButton.Size = UDim2.new(0, 82, 0, 82)
ImageButton.Image = "http://www.roblox.com/asset/?id=15411807869"
